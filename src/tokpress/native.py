...
from .profiles import NUM_PROFILES, TIKTOKEN_VOCAB_TYPE, default_registry


class TokPressCodec:
    def __init__(self) -> None:
        self.encoder_raw = TokPressEncoder(0)
        self.encoders = {profile_id + 1: TokPressEncoder(profile_id + 1) for profile_id in range(NUM_PROFILES)}
    ...
    def compress(self, data: bytes, vocab: str = "general") -> bytes:
        vocab_type = default_registry.vocab_type_for_name(vocab)
        if vocab_type > 0:
